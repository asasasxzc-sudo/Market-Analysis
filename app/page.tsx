    ...
    <div className="min-h-screen bg-[#050505] font-mono text-zinc-500 selection:bg-blue-500/30">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/5 bg-black/90 px-8 py-3 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <a href="https://ymtea.club" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
          ...
          <div className="flex gap-8 text-[9px] font-bold uppercase tracking-[0.2em]">
            <button onClick={() => scrollToSection('market-index')} className="hover:text-blue-500">Metrics</button>
            <button onClick={() => scrollToSection('keyword-analysis')} className="hover:text-blue-500">Keywords</button>
      ...
      <main className="mx-auto max-w-4xl px-6 py-24">
        ...
        <section className="mb-32 text-center">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/5 bg-white/[0.02] px-3 py-1 text-[9px] font-bold tracking-widest uppercase text-blue-500">
            <Activity size={10} /> Market terminal v2.1
          </div>
          ...
          <div className="mx-auto flex max-w-xl border border-white/10 bg-zinc-900/40 p-1 focus-within:border-blue-600/50">
            <input 
              type="text" 
            ...
            <button 
              ...
              disabled={analyzing}
              className="bg-blue-600 px-8 text-[10px] font-black uppercase tracking-widest text-white hover:bg-blue-500 disabled:opacity-50"
            >
          ...
          <div ref={resultRef} className="space-y-6 animate-in fade-in duration-500">
            ...
            <div id="market-index" className="grid grid-cols-1 gap-px bg-white/5 border border-white/5 md:grid-cols-3">
              ...
              <div className="bg-[#0A0A0A] p-8">
                ...
                <div className="text-2xl font-black text-white italic uppercase">{dynamicData.level}</div>
              </div>
            ...
            <div id="keyword-analysis" className="bg-[#0A0A0A] border border-white/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                {dynamicData.keywords.map((k, i) => (
                  <div key={i} className="flex items-center justify-between border-white/5 p-6 border-b odd:border-r hover:bg-white/[0.01]">
                    <span className="text-xs font-black text-zinc-200 tracking-widest uppercase">{k.word}</span>
                    <div className="flex items-center gap-6 text-right">
                      <div>
                        <div className="text-[7px] text-zinc-700 uppercase font-bold">Diff</div>
                        <div className="text-[9px] font-black text-zinc-500 uppercase">{k.diff}</div>
                      </div>
                      <div>
                        <div className="text-[7px] text-zinc-700 uppercase font-bold">Trend</div>
                        <div className="text-[9px] font-black text-blue-600 uppercase">{k.trend}</div>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div id="roadmap" className="bg-white p-10 text-black shadow-2xl relative">
              ...
              <h3 className="mb-12 text-[10px] font-black uppercase tracking-[0.5em]">Roadmap.Intelligence</h3>
              <div className="space-y-6">
                {dynamicData.suggestions.map((s, i) => (
                  <div key={i} className="flex gap-8 group">
                    <span className="text-2xl font-black text-zinc-200 group-hover:text-blue-600">0{i + 1}</span>
                    <div className="flex-1 border-b border-zinc-100 pb-6 group-last:border-0">
                      <p className="text-[11px] font-bold uppercase tracking-tight text-zinc-900">{s}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <footer className="border-t border-white/5 py-24 text-center">
        <p className="text-[8px] font-bold uppercase tracking-[0.8em] text-zinc-800">
          Terminal.Sync: 2026 · <a href="https://ymtea.club" className="text-zinc-700 hover:text-blue-600">YMTEA.CLUB</a>
        </p>
      </footer>
    </div>
  );
